...
public class UsersQueueExtension implements
  ...
  static {
    ...
    WITH_INCOME_REQUEST_USERS.add(new StaticUser("frog", "12345", null, "dog", null));
    WITH_OUTCOME_REQUEST_USERS.add(new StaticUser("dog", "12345", null, null, "frog"));
    WITH_FRIEND_USERS.add(new StaticUser("cat", "12345", "duck", null, null));
  }
    ...
    Arrays.stream(context.getRequiredTestMethod().getParameters())
        ...
        .forEach(ut -> {
          ...
          user.ifPresentOrElse(
              u ->
                      ((Map<UserType, StaticUser>)
                  context.getStore(NAMESPACE).getOrComputeIfAbsent(
                      context.getUniqueId(), key -> new HashMap<>())).put(ut, u),
              () -> {
  ...
  public StaticUser resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext) throws ParameterResolutionException {
      Map<UserType, StaticUser> userMap = extensionContext.getStore(NAMESPACE).get(
              extensionContext.getUniqueId(), Map.class);
      return userMap.get(AnnotationSupport.findAnnotation(parameterContext.getParameter(), UserType.class).orElseThrow(
              () -> new ParameterResolutionException("Cant find annotation UserType")
      ));
  }
}